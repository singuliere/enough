...
def test_icinga_api_hosts(host):
    ...
    r = sloppy_get(
        'https://{address}:5665/v1/objects/hosts/wereport-host'.format(
            address=address),
    ...
    assert answer['results'][0]['name'] == 'wereport-host'


def test_icinga_api_services(host):
    address = get_master_address(host)
    r = sloppy_get(
        'https://{address}:5665/v1/objects/services?host=wereport-host'.format(
            address=address),
        {'Accept': 'application/json'},
        get_auth(host),
    )
    ...
    assert len([s for s in answer['results']
                if 'wereport-host!wereport-host' == s['name']]) == 1
    assert len([s for s in answer['results']
                if 'wereport-host!wereport-host over Tor' == s['name']]) == 1
