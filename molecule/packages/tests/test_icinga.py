...
def test_icinga_api_hosts(host):
    ...
    r = sloppy_get(
        'https://{address}:5665/v1/objects/hosts/packages-host'.format(
            address=address),
    ...
    assert answer['results'][0]['name'] == 'packages-host'


def test_icinga_api_services(host):
    address = get_master_address(host)
    r = sloppy_get(
        'https://{address}:5665/v1/objects/services?host=packages-host'.format(
            address=address),
        {'Accept': 'application/json'},
        get_auth(host),
    )
    ...
    assert len([s for s in answer['results']
                if 'packages-host!Packages' == s['name']]) == 1
