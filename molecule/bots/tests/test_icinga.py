...
def test_icinga_api_hosts(host):
    ...
    r = sloppy_get(
        'https://{address}:5665/v1/objects/hosts/bots-host'.format(
            address=address),
    ...
    assert answer['results'][0]['name'] == 'bots-host'


def test_icinga_api_services(host):
    address = get_master_address(host)
    r = sloppy_get(
        'https://{address}:5665/v1/objects/services?host=bots-host'.format(
            address=address),
        {'Accept': 'application/json'},
        get_auth(host),
    )
